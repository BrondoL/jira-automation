...
class JiraRepository():
    ...
    def find_tickets_in_progress(self):
        query = f'NOT status = "{Status.DONE.value}" order by assignee ASC, priority,status,customfield_10028 desc'
    ...
    def find_tickets_done_this_month(self):
        query = f'status = "{Status.DONE.value}" AND updated >= startOfMonth() AND updated <= endOfMonth() AND "Story Points" is not empty order by assignee asc'
